...
export default function HomePage({ onNavigate }) {
  ...
  const FEATURES = [
    { icon: <Ico.Bot />,    grad: "linear-gradient(135deg,#1d8cf8,#0066cc)", glow: "rgba(29,140,248,0.35)", title: "AI Policy Assistant",   desc: "Natural language interface to Maharashtra Development Control Rules, NMC Bye-Laws, and government policy documents.",                                              tags: ["NLP","Real-time","Cited Responses"], nav: "chat" },
    { icon: <Ico.Map />,    grad: "linear-gradient(135deg,#00c48c,#009966)", glow: "rgba(0,196,140,0.35)",  title: "Zoning Intelligence",    desc: "Interactive geographic map of Nashik with color-coded zoning districts, FAR limits, height rules, and permit requirements.",                               tags: ["Interactive Map","6 Zone Types","Live Data"],   gtag: true, nav: "map" },
    { icon: <Ico.Star />,   grad: "linear-gradient(135deg,#1d8cf8,#00c48c)", glow: "rgba(0,196,140,0.3)",   title: "Scheme Eligibility",     desc: "AI-powered matching engine that analyses your profile against every active central and state government scheme.",                                           tags: ["Profile Matching","234 Schemes","Auto-Ranked"], gtag: true, nav: "schemes" },
    { icon: <Ico.Bar />,    grad: "linear-gradient(135deg,#9b7fea,#1d8cf8)", glow: "rgba(155,127,234,0.35)",title: "Authority Dashboard",    desc: "Real-time analytics console for government officials — query trends, zone popularity, scheme interest, and citizen activity.",                           tags: ["Bento Grid","Live Charts","Export-Ready"], nav: "dashboard" },
    ...
    <div className="home">
      ...
      <div className="hero">
        ...
        <div className="hero-btns">
          <button className="btn-primary" onClick={() => onNavigate("chat")}>
            <Ico.Zap /> Explore the Platform <Ico.ArrowR />
          </button>
          <button className="btn-secondary" onClick={() => onNavigate("map")}>
            <Ico.Map /> View Zoning Map
          </button>
        </div>
      ...
      <div className="section">
        ...
        <div className="features-grid">
          {FEATURES.map((f, i) => (
            <div key={i} className={`feat-card${f.nav ? " feat-card--nav" : ""}`} onClick={() => f.nav && onNavigate(f.nav)}>
              <div className="feat-icon" style={{
                ...
                <svg width={22} height={22} viewBox="0 0 24 24" fill="none"
                  stroke={f.grad.includes("00c48c") ? "#00c48c" : "#1d8cf8"}
              ...
              {f.nav && <div className="feat-cta">Open →</div>}
            </div>
          ))}
        </div>
      </div>
